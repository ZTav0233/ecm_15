...
export class ActionButtonComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
      ...
      this.actions.map((action, index) => {
        if (action === 'Archive') {
          this.tieredItems.push({
            label: 'Archive',
            icon: 'fa fa-fw ui-icon-archive', command: (event) => {
          ...
          this.tieredItems.push({
            label: 'Add-User',
            icon: 'fa fa-fw ui-icon-person-add', command: (event) => {
    ...
    if (this.activePage === 'inbox') {
      this.actions.map((action, index) => {
        if (action === 'Finish') {
          ...
          this.tieredItems.push({
            label: 'Reply',
            icon: 'fa fa-fw fa fa-fw ui-icon-reply', command: (event) => {
          ...
          this.tieredItems.push({
            label: 'Reply-All',
            icon: 'fa fa-fw fa fa-fw ui-icon-reply-all', command: (event) => {
          ...
          this.tieredItems.push({
            label: 'Forward',
            icon: 'fa fa-fw fa fa-fw ui-icon-forward', command: (event) => {
          ...
          this.tieredItems.push({
            label: 'Relaunch',
            icon: 'fa fa-fw fa fa-fw ui-icon-near-me', command: (event) => {
              this.selectedAction.emit('Relaunch');
            },
            disabled: this.disableAction || this.selectedItemLength > 1 
          });
          ...
          this.tieredItems.push({
            label: 'Flag',
            icon: 'fa fa-fw fa fa-fw ui-icon-flag', command: (event) => {
      ...
      this.tieredItems.push({
        label: 'Filter',
        icon: 'fa fa-fw fa fa-fw ui-icon-filter-list', command: (event) => {
          this.toggleFilter.emit('toggled');
        },
        disabled: (!(this.totalTableRecords > 0 && !this.disableFilter))
      });
    ...
    if (this.activePage === 'archive') {
      this.tieredItems.push({
        label: 'Un-Archive',
        icon: 'fa fa-fw fa fa-fw ui-icon-unarchive', command: (event) => {
    ...
    if (this.activePage !== 'draft') {
      if (this.hasFilterRes) {
        this.tieredItems.push({
          label: 'Reset',
          icon: 'fa fa-fw fa fa-fw ui-icon-autorenew', command: (event) => {
            if (this.activePage === 'archive') {
