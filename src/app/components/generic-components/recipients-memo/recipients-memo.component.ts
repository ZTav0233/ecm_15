...
@Component({
  ...
  styleUrls: ['recipients-memo.component.css']
})
export class RecipientsMemoComponent implements OnDestroy {
  ...
  addToCCListFromList(role) {
    if (!this.existsInList(role)) {
      ...
      role.disabled = true;
      console.log(this.recipientsData);
      this.recipientsData.ccList.push(role);
      this.prepareStepItems.emit();
    }
  ...
  onRecipientRemoved(item?:any) {
    console.log("removed")
      console.log("item", item)
      this.prepareStepItems.emit();
      this.cancellingItem.emit(item)
